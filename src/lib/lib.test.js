...
test("runRules flags over-limit meals as critical", () => {
  const r = runRules(
    { id: "a", extracted: { vendor: "Taj", amount: 3000, date: "15/01/2026", category: "meals" } },
...
test("runRules flags missing required fields", () => {
  ...
  assert.ok(r.violations.some((v) => v.rule === "required_field" && v.detail.includes("vendor")));
});

test("runRules detects duplicate receipt", () => {
  const r = runRules(
    { id: "b", extracted: { vendor: "X", amount: 500, date: "01/01/2026", category: "meals" } },
    { effectiveFromDate: new Date("2025-01-01") },
...
test("runRules flags pre-effective-date receipt", () => {
  const r = runRules(
    { id: "a", extracted: { vendor: "X", amount: 100, date: "01/01/2024", category: "meals" } },
    { effectiveFromDate: new Date("2025-01-01") },
    [],
  );
...
test("computeRiskScore inverts confidence and weights severity", () => {
  const s = computeRiskScore({
    rulesOutput: { severity: "critical", violations: [{ rule: "x", severity: "critical" }] },
